...
LUA_SHA1 = get_hash(os.path.join(CWD, 'picdig.lua'))
    ...
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'project':
                user_name, project_id = item_value.split(':', 1)
                wget_args.extend(['--warc-header', 'picdig-project-id: '+project_id])
                wget_args.append('https://picdig.net/api/users/{}/projects/{}'.format(user_name, project_id))
            elif item_type == 'column':
                user_name, column_id = item_value.split(':', 1)
                wget_args.extend(['--warc-header', 'picdig-column-id: '+column_id])
                wget_args.append('https://picdig.net/api/users/{}/columns/{}'.format(user_name, column_id))
            elif item_type == 'user':
                wget_args.extend(['--warc-header', 'picdig-user-id: '+item_value])
                wget_args.append('https://picdig.net/api/v2/users/'+item_value)
            elif item_type == 'cdn':
...
project = Project(
    title=TRACKER_ID,
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/7/76/Picdig-icon.jpg" height="50px" title=""/>
        <h2>picdig <span class="links"><a href="https://picdig.net/">Website</a> &middot; <a href="http://tracker.archiveteam.org/picdig/">Leaderboard</a> &middot; <a href="https://wiki.archiveteam.org/index.php/picdig">Wiki</a></span></h2>
        <p>Archiving picdig.</p>
    '''
)
